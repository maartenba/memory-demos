...
public static class BeerLoader
{
    ...
    public static void LoadBeersInsane()
    {
        ...
        var jsonArray = JsonNode.Parse(json) as JsonArray; // store it a second time (as JsonArray)
        foreach (var token in jsonArray)
        {
            var beer = token as JsonNode;
            if (beer != null)
            {
                var breweryName = beer["brewery"].GetValue<string>();
                var beerName = beer["name"].GetValue<string>();
                var rating = beer["rating"].GetValue<double>();
        ...
        // string and JsonArray out of scope, memory traffic, expect a GC here...
    }
    ...
    public static void LoadBeersUnoptimized()
    {
        ...
        // note: System.Text.Json does not allow streaming from a file (does support reading a stream in one go),
        // so some plumbing is needed here with this custom class
        using var reader = new Utf8JsonStreamReader(File.OpenRead(Path.Combine("Demo03", "beers.json")), 32 * 1024);
        while (reader.Read())
        {
            if (reader.TokenType == JsonTokenType.StartObject)
            {
                // Load object from the stream
                var beer = reader.Deserialize<JsonObject>();
                reader.Read();
